...
impl<O, T, const N: usize> Parser<O> for [T; N]
where
    T: Parser<O>,
{
...
pub trait Parser<O> {
    fn parse(self, input: &mut Tokens) -> Result<O, ParseError>;
    fn map<S, F>(self, f: F) -> impl Parser<S>
    where
        F: Fn(O) -> S,
    {
...
fn lustre_parser(input: &str) -> Result<Ast, ParseError> {
    let lexer = Token::lexer(input);
    let tokens = Tokens { lexer, buf: vec![] };
    todo!()
}
fn ast(input: &mut Tokens) -> Result<Ast, ParseError> {
    let a = [Token::Node, Token::ParenOpen, Token::ParenClose]
        .map(|s| {})
        .parse(input);
    todo!()
}
...
mod test {
    ...
    fn test() {
        ...
        let mut iterator = Token::lexer(input);
