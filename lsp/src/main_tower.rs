...
impl LanguageServer for Backend {
    ...
    async fn formatting(&self, _: DocumentFormattingParams) -> Result<Option<Vec<TextEdit>>> {
        match self.data.read().await.formatting() {
            Some(s) => Ok(Some(s)),
            None => Ok(None),
        }
    }
    ...
    async fn document_highlight(
        ...
        match self.data.read().await.document_hightlight(pos) {
            Some(s) => Ok(Some(s)),
            None => Ok(None),
        }
    }
    ...
    async fn diagnostic(
        ...
        Ok(self.data.read().await.diagnostic())
    }
    ...
    async fn semantic_tokens_full(
        ...
        match self.data.read().await.semantic_tokens_full() {
            Some(s) => Ok(Some(s)),
            None => Ok(None),
        }
    }
    async fn inlay_hint(&self, _: InlayHintParams) -> Result<Option<Vec<InlayHint>>> {
        match self.data.read().await.inlay_hint() {
            Some(s) => Ok(Some(s)),
            None => Ok(None),
        }
    }
